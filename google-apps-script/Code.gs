...
function doPost(e) {
  ...
  switch (action) {
    ...
    case "clear_progress":
      return jsonOutput(clearProgress_(payload));
    case "dashboard":
      return jsonOutput({
        ok: true,
        users: getUsers_(),
...
function registerUser_(payload) {
  ...
  usersSheet.appendRow([
    ...
    0,
    "-",
    JSON.stringify(normalizeStats_({}))
  ]);
...
function saveScore_(payload) {
  ...
  const stats = normalizeStats_(user.stats || {});
...
function incrementError_(payload) {
  ...
  const stats = normalizeStats_(user.stats || {});
  stats.errorsByPhase[phase] = (stats.errorsByPhase[phase] || 0) + 1;
...
function clearProgress_(payload) {
  ...
  stats.currentPhase = 1;
  stats.hasSavedProgress = false;

  usersSheet.getRange(rowIndex, 6, 1, 2).setValues([[1, Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "dd/MM/yyyy")]]);
  usersSheet.getRange(rowIndex, 8).setValue(JSON.stringify(stats));
...
function getUsers_() {
  ...
  return values.slice(1).filter((row) => row[1]).map((row) => {
    const stats = normalizeStats_(safeParse_(row[7]));
    return {
